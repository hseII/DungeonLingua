...
def handle_room_transition(target_room_id, dungeon_data):
    ...
    # Проверка, является ли комната комнатой с финальным сокровищем
    if target_room.get("treasures"):
        for treasure in target_room["treasures"]:
            if treasure.get("is_final_goal", False):
                print("HELL YEAHH")  # Условие конца игры
                st.session_state.game_over = True  # Устанавливаем флаг завершения игры
                st.rerun()  # Перезагружаем страницу для отображения результата
    ...
    st.session_state.npc_dialog_completed = False
    st.session_state.npc_success = False
...
def render():
    ...
    with header_cols[2]:
        if current_room.get('npcs'):
            ...
            for idx, npc in enumerate(current_room['npcs']):
                with st.container():
                    if st.button(f"Chatter Mead", key=f"npc_{idx}"):
    ...
    if st.session_state.current_screen == "npc" and st.session_state.selected_npc:
        ...
        if current_room["room_type"] == "trap":
            ...
            # Рендер дверей, открытых через NPC
            ...
            if unlocked_doors:
                ...
                for idx, room_id in enumerate(unlocked_doors):
                    target_room = find_room_by_id(st.session_state.dungeon_data, room_id)
                    with cols[idx]:
                        if st.button(f" {target_room['name']}", key=f"npc_door_{room_id}"):
                            handle_room_transition(room_id, st.session_state.dungeon_data)
                ...
                if current_room.get('npcs'):
                    ...
                    for idx, (col, exit_data) in enumerate(zip(cols, exits)):
                        with col:
                            ...
                            target_room = find_room_by_id(st.session_state.dungeon_data, target_room_id)
                            ...
                            if puzzle_met or is_visited:
                                ...
                                if col.button(target_room['name'], key=f"exit_{idx}"):
                                    handle_room_transition(target_room_id, st.session_state.dungeon_data)
